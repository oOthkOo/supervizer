...
function updateAppStats( app ) {
	...
	if (app.status == 'online') {
		...
		usage.lookup(app.pid, { keepHistory: true }, function(err, result) {
			app.stats.cpu = result.cpu.toFixed(2);
			app.stats.memory = result.memory;
			//console.log( result );
		});
	}
}
...
function save( data ) {	
	...
	try {
		fs.writeFileSync(data.file, JSON.stringify(apps));		
	}
	catch (e) {
...
function add( app ) {
	...
	return getSucess( app, 'App (' + app.name + ') succesfully added.' );
}
...
function remove( appToRemove ) {
	...
	return getSucess( null, 'App (' + app.name + ') succesfully removed.' );
}
...
function set( data ) {
	...
	return getSucess( app, 'App (' + app.name + ') succesfully updated.' );
}
...
function get( data ) {
	...
	return getSucess( app, 'App (' + app.name + ') properties succesfully retrieved.' );
}
...
function start( appToStart ) {
	...
	return getSucess( app, 'App (' + app.name + ') succesfully started.' );
}
...
function stop( appToStop ) {	
	...
	return getSucess( app, 'App (' + app.name + ') succesfully stopped.' );
}
