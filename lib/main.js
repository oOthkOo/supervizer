...
function showInfo (message, name) {
	showLog('[INF]: ' + (name ? '(' + name + ') ' : '') + message )
}

function showError (message, name) {
	showLog('[ERR]: ' + (name ? '(' + name + ') ' : '') + message )
}
...
function updateAppStats(app) {
	// Update app stats
	if (app.status == 'up') {
		pidusage(app.pid, (err, stats) => {
			if (err) {
				return
			}
			app.stats.cpu = stats.cpu ? stats.cpu.toFixed(2) : 0
			app.stats.memory = stats.memory ? stats.memory : 0
		})
	}
}
...
function startApp (app) {
	...
	if (app.uid ||
		...
		if (app.uid) {
			// Convert to number
			options.uid = parseInt(app.uid)
		}
		if (app.gid) {
			// Convert to number
			options.gid = parseInt(app.gid)
		}
	}
